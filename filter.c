...
int is_filter_clause_valid(const json_t * input_filter_clause) {
  ...
  if (input_filter_clause == NULL || !json_is_object(input_filter_clause)) {
    ...
    if (filter_clause_value == NULL) {
      ...
      if (0 == strcmp("date", element_value) || 0 == strcmp("priority", element_value)) {
        if (!json_is_integer(filter_clause_value)) {
          return 0;
        } else if (0 == strcmp("date", element_value) && json_integer_value(filter_clause_value) < 0) {
          return 0;
        } else if (0 == strcmp("priority", element_value) && ((json_integer_value(filter_clause_value) < 0) || json_integer_value(filter_clause_value) > NB_PRIORITY)) {
          return 0;
        }
      } else if (0 == strcmp("source", element_value) || 0 == strcmp("message", element_value) || 0 == strcmp("tag", element_value)) {
...
json_t * parse_filter_clause_from_http(const json_t * filter_clause) {
  ...
  if (filter_clause != NULL && json_is_object(filter_clause) && to_return != NULL) {
    ...
    } else if (0 == strncasecmp(json_string_value(json_object_get(filter_clause, "operator")), "contains", strlen("contains"))) {
      json_object_set_new(to_return, COLUMN_FILTER_CLAUSE_OPERATOR, json_integer(FILTER_CLAUSE_OPERATOR_CONTAINS));
    } else if (0 == strncasecmp(json_string_value(json_object_get(filter_clause, "operator")), "notcontains", strlen("notcontains"))) {
